...
class ContadorBigramas {
 public:
  ...
  /**
     @brief suma uno a la frecuencia del bigrama pasada por su posicion en el bigrama
     @param pos: "cordenadas" del bigrama
     @return @retval true si ha tenido éxito @retval false en otro casol
     @warnning este método ha sido añadido por Blanca 
   */
  
  bool addBigrama( const int pos[]);
  /**
  ...
  /**
     @brief Modifica vector con posiciones de los caracteres válidos
     @return @retval true si el caracter se encuentra entre los válidos, false en caso contrarios
     @warning método añadido por Blanca 
   */
  bool posiciones (const char cadena[] , int *pos);

  /**
     @brief Devueve la posición de cierto caracter
     @param a: susodicho caracter
     @return posición, -1 si no se encuentra en el caraceres
   */

  int correcto( const char a);
 ...
 private:
  ...
  int  _nc; /// número de caracteres válidos
